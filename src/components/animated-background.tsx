...
const AnimatedBackground = () => {
  ...
  const keyboardStates = React.useCallback((section: Section) => {
    return STATES[section][isMobile ? "mobile" : "desktop"];
  }, [isMobile]);

  const handleMouseHover = React.useCallback((e: SplineEvent) => {
  ...
  const handleSplineInteractions = React.useCallback(() => {
    if (!splineApp) return;
  ...
  const handleGsapAnimations = React.useCallback(() => {
    if (!splineApp) return;
  ...
  const getBongoAnimation = React.useCallback(() => {
  ...
  const getKeycapsAnimation = React.useCallback(() => {
  ...
  const revealKeyCaps = React.useCallback(async () => {
    if (!splineApp) return;
